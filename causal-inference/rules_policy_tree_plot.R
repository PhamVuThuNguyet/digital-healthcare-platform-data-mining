...
number_to_percentage <- function(num) {
  paste0(formatC(num * 100, format = "f", digits = 1), "%")
}
...
#' @keywords internal
create_dot_body_complex <- function(tree, index = 1) {
  ...
  if (node$is_leaf) {
    ...
    line_label <- paste(index - 1,
                        ' [shape=Mrecord,style=filled, label= <<TABLE BORDER="0" CELLBORDER="0" CELLSPACING="0">/n<TR>/n<TD><b>',
                        tree$leaf.labels[action], '</b></TD>/n</TR>/n<TR>/n<TD>', 
                        round(node[['stats']][['estimate']], 3), #ate rounded to 3dp
                        stars_for_pvalue(node[['stats']][['p']]), #significant figures
                        '</TD>/n</TR>/n<TR>/n<TD>',
                        number_to_percentage(node[['stats']][['sample_p']]), #TO_DO : SAMPLE PERCENTAGE,
                        '</TD>/n</TR>/n</TABLE>>',
                        '];')
    return(line_label)
  ...
  node_info <- paste(index - 1,
                      ' [shape=box, label= <<TABLE BORDER="0" CELLBORDER="0" CELLSPACING="0">/n<TR>/n<TD><b>',
                     variable_name, '&le;', round(node$split_value, 3),
                     '</b></TD>/n</TR>/n<TR>/n<TD>', 
                      round(node[['stats']][['estimate']], 3), #ate rounded to 3dp
                      stars_for_pvalue(node[['stats']][['p']]), #significant figures
                      '</TD>/n</TR>/n<TR>/n<TD>',
  ...
  left_child_lines <- ifelse(!is.null(node$left_child),
                             create_dot_body_complex(tree, index = node$left_child),
                             NULL
  )
  
  right_child_lines <- ifelse(!is.null(node$right_child),
                              create_dot_body_complex(tree, index = node$right_child),
                              NULL
  )
...
export_graphviz <- function(tree) {
  ...
  footer <- "}"
  body <- create_dot_body_complex(tree)
...
#' }
#' @export
plot_policy_tree <- function(x, leaf.labels = NULL, ...) {
