cal.pvalue <- function(ATE) { # nolint
    # Sample data
    sample_mean <- ATE[1] # Replace with your sample mean
    sample_sd <- ATE[2] # Replace with your sample standard deviation
    n <- ATE[3] # Replace with your sample size
    null_mean <- 0 # Replace with your null hypothesis mean

    # Calculate the t-statistic # nolint
    ...
    return(c(ATE, t.stat = t_statistic, p_val = p_value, lb = lb, ub = ub))
}


cal.pvalue.df <- function(ATE) {
    # Sample data
    sample_mean <- ATE$estimate # Replace with your sample mean
    sample_sd <- ATE$std.err # Replace with your sample standard deviation
    n <- ATE$sample_size # Replace with your sample size
    null_mean <- 0 # Replace with your null hypothesis mean
...
#' @export
average_treatment_effect_w_ss <- function(forest,
    ...
    if (target.sample == "overlap") {
        ...
        return(c(estimate = tau.est, std.err = tau.se, sample_size = length(subset)))
    }
    ...
    return(c(estimate = tau.avg, std.err = tau.se, sample_size = length(subset)))
}
